document.addEventListener('DOMContentLoaded', () => {
  ...
  const initMenuPickers = (root = document) => {
    const forms = root.querySelectorAll('.menu_configurator_form');
    forms.forEach((form) => {
      if (form.dataset.menuPickerReady) {
        return;
      }
      form.dataset.menuPickerReady = 'true';
      form.addEventListener('click', onFormClick);
      form.addEventListener('click', onChipRemove);
    });

    if (!document.body.dataset.menuPickerGlobal) {
      document.body.dataset.menuPickerGlobal = 'true';
  ...
  window.initMenuPickers = initMenuPickers;
  initMenuPickers();
});
